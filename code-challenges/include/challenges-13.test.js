...
const onlyOddChars = (str) => {
  str = str.split('')
  let output = str.filter((element)=>{
    if(str.indexOf(element)%2!=0){
      return element
    }
  })
  return output.join('')
...
const unenrollBrook = (arr) => {
  let output = arr.map(list =>{
    let checker = list.filter(string =>{
      if(!(string.includes('Brook'))){
        return string
      }
    })
    return checker
