...
const count = (target, input) => {
  let output = input.reduce((ccr,element) =>{
    let processes = element.filter(number =>{
      if(number === target){
        return number
      }
    })
    ccr = ccr + processes.length
    return ccr
  },0) 
  return output 
};
...
const totalSum = (input) => {
  let total = input.reduce((ccr,list)=>{
    let output = list.reduce((cr,number)=>{
      cr = cr + number
      return cr
    },0)
    ccr = ccr + output 
...
const divisibleByFiveTwoToThePower = (input) => {
  let output = input.map(element => {
    ...
    // console.log(element)
    return element = element.map(number => {
...
let findMaleAndFemale = (data) => {
  let output = ''
  data.forEach(star =>{
    if(star.gender === 'female' || star.gender === 'male'){
      if(data.indexOf(star) === 0){
        output = output + `${star.name}`
      }else {
      output = output + ' and '+ `${star.name}`
    }
  }
  })
  return output
...
let findShortest = (data) => {
  ...
  data.map(star =>{
    if(shorter === 0){
      name_shorter = star.name
      shorter = star.height
    }if(parseInt(star.height) < shorter){
      name_shorter = star.name
      shorter  = star.height
    }
  })
